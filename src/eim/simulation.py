...
def run_tri_lobe_eim_simulation(max_steps=400, eta=0.05, num_cells=5, kappa=143.9,
                              observer_defect_strength=15.0, scar_increment=0.08,
                              gw_step=None, gw_strength=8.0, gw_radius=4, show_plots=True):
    ...
    for step in range(max_steps):
        ...
        switches = update_evap_state(G, node_data, node_evap_state, scar_increment=scar_increment)
        ...
        if gw_step is not None and step == gw_step:
            ...
            inject_gravitational_wave(G, center, gw_strength, gw_radius)
            print(f"GW injected at step {step}")
        ...
        if (step + 1) % 100 == 0:
            print(f"Step {step+1}: Commits={seam_commits}, SeamFrac={seam_fractions[-1]:.3f}")
    ...
    if show_plots:
        ...
        axes[2].plot(seam_fractions); axes[2].set_title('Seam Fraction')
        axes[3].plot(signed_branch_means); axes[3].set_title('Signed Branch Mean')
        axes[4].plot(p_rho3_norms, label='P_ρ₃')
        axes[4].plot(p_rho3p_norms, label="P_ρ₃'")
        axes[4].legend()
        axes[4].set_title('Memory Norms')
        plt.tight_layout()
        plt.show()

    print(f"\nTotal OBS Seam Commits: {seam_commits}")
